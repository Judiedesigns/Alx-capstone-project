...
function Science() {
        ...
        const fetchScienceNews = async () => {
            try {
                const res = await axios.get(
                    `https://api.thenewsapi.com/v1/news/all?api_token=${api_token}&language=en&categories=science`
                );
                // console.log(res.data.data);
                setScienceNews(res.data.data);
            } catch (err) {
                console.error(err);
    ...
    return (
        <div>
            ...
            {scienceNews.length === 0 && !loading && !error && (
                <div className="text-center mt-20 text-gray-500 text-lg">No science news articles available at the moment.</div>
            )}
